...
public class VenteGameState extends BasicGameState {
	...
	public VenteGameState(Jeu j,Inventaire inven,JournalQuete jourq){
		 jeu=j;
		 inv=inven;
		 jq=jourq;
	 }
	    ...
	    case 0 :
	    	...
	    	g.drawString("<- Objets Utilisables ->", 100, 75);
	    	    ...
	    	    if(inv.NombreUtilisablePresent()>0){
	    	    	...
	    	    	g.setColor(Color.black);
	    	    	int id = inv.getUtilisablePresent(j).getidObjet();
	    	    	g.drawString(inv.getUtilisablePresent(j).getnomObjet(), 500, 150);
	    	    	g.drawString("Quantite possedee : " + inv.getquantiteObjet(id), 500, 175);
	    	    	g.drawString("Valeur : " + inv.getObjetPresent(j).getvaleur(), 500, 200);
	    	    	if (0<=id && id<20)
	    	    		{
	    	    			g.drawString("Objet utilisable en combat", 500, 250);
	    	    			if (id<5) g.drawString("Montant vie : " + ((ObjetUtilisable) inv.getUtilisablePresent(j)).getmontantVie() , 500, 275);
	    	    			if (id>=5 && id<10) g.drawString("Montant mana : " + ((ObjetUtilisable) inv.getUtilisablePresent(j)).getmontantMana() , 500, 275);
	    	    			if (id>=10 && id<15) g.drawString("Montant attaque : " + ((ObjetUtilisable) inv.getUtilisablePresent(j)).getmontantAttaque() , 500, 275);
	    	    			if (id>=15 && id<20) g.drawString("Montant defense : " + ((ObjetUtilisable) inv.getUtilisablePresent(j)).getmontantDefense() , 500, 275);
	    	    		}
	    	    	g.setColor(Color.white);
	  ...
	  public void keyPressed(int key, char c) {
		  ...
		  case Input.KEY_LEFT:
			  ...
			  if (choixq == 1){
				  ...
				  switch (onglet){
				  	case 0 :
				  		qmaxv = inv.getquantiteObjet(inv.getUtilisablePresent(pointeur).getidObjet());
					  break;
				  	case 1 :
				  		qmaxv = inv.getquantiteObjet(inv.getEquipablePresent(pointeur).getidObjet());
						  break;
				  	case 2 :
				  		qmaxv = inv.getquantiteObjet(inv.getAutrePresent(pointeur).getidObjet());
				  		break;
				  	}
					  if (quantite == 0){quantite = qmaxv;} else {quantite = quantite - 1;}
		  ...
		  case Input.KEY_RIGHT:
			  ...
			  if (choixq == 1){
				  	...
				  	switch (onglet){
				  	case 0 :
				  		 qmaxv2 = inv.getquantiteObjet(inv.getUtilisablePresent(pointeur).getidObjet());
					  break;
				  	case 1 :
				  		 qmaxv2 = inv.getquantiteObjet(inv.getEquipablePresent(pointeur).getidObjet());
						  break;
				  	case 2 :
				  		 qmaxv2 = inv.getquantiteObjet(inv.getUtilisablePresent(pointeur).getidObjet());
				  		break;
				  	} 
					  if (quantite == qmaxv2){quantite = 0;} else {quantite = quantite + 1;}
			  ...
			  case 0 :
				  if (inv.NombreUtilisablePresent() > 0){
		  			...
		  			case 0 :
					  	...
					  	{choixq = 1;} else{choixq = 2;}
			  			break;
		  			case 1 :
		  					int idobj = inv.getUtilisablePresent(pointeur).getidObjet();
		  					inv.vendre(inv.getUtilisablePresent(pointeur),quantite,jq);
			  ...
			  case 1 :
				  if (inv.NombreEquipablePresent() > 0){
		  			...
		  			case 0 :
			  			...
			  			break;
		  			case 1 :
		  					int idobj = inv.getEquipablePresent(pointeur).getidObjet();
		  					inv.vendre(inv.getEquipablePresent(pointeur),quantite,jq);
		  					if (inv.getquantiteObjet(idobj)==0) 
			  ...
			  case 2 :
				  if (inv.NombreAutrePresent() > 0){
		  			...
		  			case 1 :
		  					...
		  					inv.vendre(inv.getAutrePresent(pointeur),quantite,jq);
		  					if (inv.getquantiteObjet(idobj)==0) 
